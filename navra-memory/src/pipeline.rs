...
/// server layer to apply PII filtering without navra-memory
/// depending on navra-security.
...
mod tests {
    ...
    impl navra_model::ModelBackend for MockModelBackend {
        fn generate(
            &self,
            _request: &navra_model::GenerateRequest,
        ) -> std::pin::Pin<
            Box<
                dyn std::future::Future<
                        Output = Result<navra_model::GenerateResponse, navra_model::ModelError>,
                    > + Send
                    + '_,
            >,
        > {
            ...
            Box::pin(async move {
                Ok(navra_model::GenerateResponse {
                    text,
    ...
    impl navra_model::ModelBackend for FailingModelBackend {
        fn generate(
            &self,
            _request: &navra_model::GenerateRequest,
        ) -> std::pin::Pin<
            ...
            >,
        > {
            Box::pin(async { Err(navra_model::ModelError::Inference("mock failure".into())) })
        }
    }
    ...
    async fn synthesize_with_failing_model_falls_back_to_stub() {
        ...
        let ks = KnowledgeStore::open_memory().unwrap();
        let model: Arc<dyn navra_model::ModelBackend> = Arc::new(FailingModelBackend);
        let pipeline = DistillationPipeline::new(&wm, &ks).with_model(model);
