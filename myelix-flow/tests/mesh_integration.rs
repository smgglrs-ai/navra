...
use myelix_protocol::label::{Confidentiality, DataLabel};
...
clearance = "public"
"#;
    let def: myelix_flow::FlowDefinition = toml::from_str(toml_str).unwrap();
...
max_iterations = 3
"#;
    let def: myelix_flow::DagDefinition = toml::from_str(toml_str).unwrap();
