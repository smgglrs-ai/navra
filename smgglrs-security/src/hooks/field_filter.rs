...
impl Hook for FieldFilterHook {
    ...
    async fn post_tool_use(
        ...
        for content in &mut filtered.content {
            if let smgglrs_protocol::Content::Text(ref mut text) = content {
...
mod tests {
    ...
    fn make_result(json_text: &str) -> CallToolResult {
        CallToolResult {
            content: vec![smgglrs_protocol::Content::text(json_text.to_string())],
            is_error: false,
            label: smgglrs_protocol::label::DataLabel::TRUSTED_PUBLIC,
        }
    }
