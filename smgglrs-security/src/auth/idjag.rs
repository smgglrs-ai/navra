...
mod tests {
    ...
    fn idjag_verify_registered_key() {
        ...
        let claims = format!(
            r#"{{"iss":"https://test.provider","sub":"agent-1","aud":"smgglrs","exp":{}}}"#,
            future_exp()
        );
    ...
    fn idjag_unknown_provider_rejects() {
        ...
        let header = r#"{"alg":"EdDSA","kid":"test-key"}"#;
        let claims = format!(
            r#"{{"iss":"https://unknown.provider","sub":"agent-1","aud":"smgglrs","exp":{}}}"#,
            future_exp()
        );
        let token = build_test_jwt(header, &claims, &signer);
    ...
    fn idjag_chain_fallthrough() {
        ...
        let jwt_claims = format!(
            r#"{{"iss":"https://test.provider","sub":"agent-2","aud":"smgglrs","exp":{}}}"#,
            future_exp()
        );
    ...
    fn idjag_authenticate_header() {
        ...
        let header = r#"{"alg":"EdDSA","kid":"test-key"}"#;
        let claims = format!(
            r#"{{"iss":"https://test.provider","sub":"agent-3","aud":"smgglrs","exp":{}}}"#,
            future_exp()
        );
        let token = build_test_jwt(header, &claims, &signer);
    ...
    fn idjag_audience_array_accepted() {
        ...
        let header = r#"{"alg":"EdDSA","kid":"test-key"}"#;
        let claims = format!(
            r#"{{"iss":"https://test.provider","sub":"agent-1","aud":["smgglrs","other"],"exp":{}}}"#,
            future_exp()
        );
        let token = build_test_jwt(header, &claims, &signer);
    ...
    fn idjag_multiple_providers() {
        ...
        let claims1 = format!(
            r#"{{"iss":"https://api.openai.com","sub":"gpt-agent","aud":"smgglrs","exp":{}}}"#,
            future_exp()
        );
        ...
        let claims2 = format!(
            r#"{{"iss":"https://api.anthropic.com","sub":"claude-agent","aud":"smgglrs","exp":{}}}"#,
            future_exp()
        );
