...
//! environment variables. Agents never see raw secrets — smgglrs
...
pub trait CredentialStore: Send + Sync {
    ...
    /// Store a credential under a label (smgglrs-managed only).
    fn store(&self, label: &str, secret: &[u8]) -> anyhow::Result<()>;

    /// Delete a credential (smgglrs-managed only).
