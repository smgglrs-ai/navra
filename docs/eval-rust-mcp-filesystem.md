...
This revealed a critical gap: upstream tool calls bypassed navra's
permission system. Filed and fixed as NAVRA-077.

### Re-test (post-NAVRA-077 — upstream tool ACL enforcement)

| Test | Expected | Result |
|---|---|---|
| Read file (readonly agent) | Allowed | **PASS** |
| Write file (readonly agent) | Blocked | **PASS** — auto-classified as write |
| Edit file (readonly agent) | Blocked | **PASS** — auto-classified as write |
| search_files_content | Allowed | **PASS** — classified as read |
| zip_files (denied by policy) | Not registered | **PASS** — "Unknown tool" |
| No pwned.txt on disk | No file created | **PASS** |

navra now auto-classifies upstream tools using MCP ToolAnnotations,
name heuristics, and operator overrides. Write-classified tools are
blocked for agents whose permission set lacks the "write" operation.
Tools with `tool_overrides = "deny"` are dropped at discovery time.
...
| zip operations could exhaust disk | Low | Use `tool_overrides` to deny zip tools |

## Deployment checklist

1. **Separate process** managed by the gateway, not inside the agent container
2. **Use `tool_overrides`** to deny zip/unzip tools unless explicitly needed
3. **Never enable `--enable-roots`** in containerized deployments
4. **Mount volumes read-only** at the container level as defense in depth
