...
impl IntegrityMonitor {
    ...
    pub async fn initialize(&mut self, embed_backend: Option<&dyn navra_model::ModelBackend>) {
    ...
    pub async fn check(&mut self, embed_backend: Option<&dyn navra_model::ModelBackend>) {
...
pub fn spawn_monitor(
    ...
    embed_backend: Option<Arc<dyn navra_model::ModelBackend>>,
...
async fn compute_embedding(
    backend: Option<&dyn navra_model::ModelBackend>,
    content: &str,
) -> Option<Vec<f32>> {
    let backend = backend?;
    let request = navra_model::EmbedRequest {
