...
mod tests {
    ...
    fn render_prometheus_format() {
        ...
        let output = m.render();
        assert!(output.contains("# TYPE smgglrs_tool_calls_total counter"));
        assert!(output.contains("smgglrs_tool_calls_total 42"));
        assert!(output.contains("smgglrs_safety_triggers_blocked_total 3"));
        assert!(output.contains("# TYPE smgglrs_sessions_active gauge"));
    }
    ...
    fn render_contains_all_metrics() {
        let m = Metrics::new();
        let output = m.render();
        assert!(output.contains("smgglrs_tool_calls_total"));
