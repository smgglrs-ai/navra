...
use navra_core::credentials::{CredentialMapping, CredentialStore, MappedCredentialStore};
...
mod forgery {
    ...
    fn malformed_token_rejected() {
        ...
        assert!(decode_token("navra_cap_v1.bad-base64!.also-bad!", &signer).is_err());
        assert!(decode_token("navra_cap_v2.aaa.bbb", &signer).is_err()); // wrong version prefix
        assert!(decode_token("navra_cap_v1.onlytwoparts", &signer).is_err());
    }
...
mod quota {
    use navra_core::quota::{QuotaEngine, RateLimit};
