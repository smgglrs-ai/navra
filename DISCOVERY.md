# Agent & Tool Discovery — Landscape and Implications for mcpd

Research captured April 2026. This document surveys the emerging
discovery mechanisms for AI agents and tools, and outlines how mcpd
...
mcpd sits at the intersection of these layers — it is both an MCP
...
### Downstream: Make mcpd Discoverable
...
2. **AID DNS record** — Publish an `_agent` TXT record pointing to
   the mcpd endpoint with `p=mcp`. For local-only deployments, the
   `.well-known/agent` JSON fallback works over the Unix socket or
   localhost TCP.

3. **A2A Agent Card** — If mcpd evolves toward A2A support, serve
...
3. **Local network discovery** — Use mDNS/DNS-SD (the `zeroconf`
   ...
   network. This is particularly relevant for mcpd's desktop-first
   deployment model.

### Security Considerations

- **AID PKA** aligns with mcpd's security posture — Ed25519 endpoint
...
- **MCP Server Cards** are static metadata and do not bypass mcpd's
