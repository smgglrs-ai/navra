...
///
/// Defined here (in navra-security) to avoid a circular dependency
/// on navra-memory. The server wires the concrete implementation.
...
impl Default for MemoryExtractionConfig {
    fn default() -> Self {
        Self {
            ...
            exclude_tools: vec!["navra_var_*".to_string(), "memory_*".to_string()],
        }
    }
}
...
mod tests {
    ...
    #[tokio::test]
    async fn skips_excluded_tools_navra_var() {
        let store = Arc::new(TestStore::new());
        let hook = make_hook(Arc::clone(&store));
