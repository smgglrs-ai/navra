//! Integration tests for navra-security public API.
//!
...
impl Hook for TestResultHook {
    ...
    async fn post_tool_use(
        ...
        let text = result.content[0].raw.as_text().expect("expected text content");
...
async fn hook_pipeline_post_modifies_result() {
    ...
    let original = navra_protocol::CallToolResult::text("output");
    let result = pipeline
...
fn capability_token_sign_verify_roundtrip() {
    ...
    assert!(token.starts_with("navra_cap_v1."));
...
fn root_payload(signer: &Ed25519Signer) -> navra_security::auth::capability::CapabilityPayload {
    build_payload(
        signer.did(),
        signer.did(),
        CapabilitySet {
...
fn asi10_trust_decay_on_safety_trigger() {
    use navra_security::trust_score::{TrustConfig, TrustScore, TrustState};
