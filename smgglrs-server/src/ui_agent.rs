...
use smgglrs_agent::{McpClient, ToolLoopConfig};
use smgglrs_cognitive::ForgeService;
use smgglrs_core::McpServer;
use smgglrs_memory::{Message, Role, Turn, WorkingMemory};
use smgglrs_model::ModelBackend;

// Re-exports from smgglrs-core for the in-process transport
use smgglrs_core::upstream::Transport;
use smgglrs_core::Upstream;
...
impl Transport for DirectTransport {
    ...
    ) -> Result<serde_json::Value, smgglrs_core::upstream::UpstreamError> {
        let method = body["method"].as_str().unwrap_or("");
        let params = body.get("params").cloned();
        let id = body.get("id").cloned().unwrap_or(serde_json::json!(1));

        let agent = smgglrs_core::auth::AgentIdentity::new("ui-agent", "dev");

        match method {
            "initialize" => {
                let init_params = smgglrs_core::protocol::InitializeParams {
                    protocol_version: smgglrs_core::protocol::PROTOCOL_VERSION.to_string(),
                    capabilities: Default::default(),
                    client_info: smgglrs_core::protocol::ClientInfo {
                        name: "smgglrs-ui-agent".to_string(),
            ...
            "tools/list" => {
                let pagination: smgglrs_core::protocol::PaginatedRequest = params
...
impl smgglrs_agent::AuditSink for StreamingAuditSink {
    fn log_tool_call(
        &self,
        _run_id: &str,
        _agent_id: &str,
...
pub(crate) async fn handle_agentic_chat(
    ...
    let system_prompt = if let Some(ref persona_name) = req.persona {
        if !persona_name.is_empty() {
            smgglrs_cognitive::assemble(&state.forge, persona_name, &req.prompt, None, None)
...
mod tests {
        ...
        fn respond(
            ...
            Box<
                dyn std::future::Future<
                        Output = Result<smgglrs_model::ModelResponse, smgglrs_model::ModelError>,
                    > + Send
                    + '_,
            >,
        > {
    ...
    fn build_test_state() -> Arc<AgentChatState> {
        let server = Arc::new(smgglrs_core::McpServer::builder().allow_anonymous().build());
