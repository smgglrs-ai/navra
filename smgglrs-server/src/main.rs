...
async fn serve(cfg: config::Config, no_tray: bool) -> anyhow::Result<()> {
    ...
    // Register team orchestration tools
    {
        ...
        // The teammate connects to smgglrs via MCP, gets its own tool loop,
        // and can use docs_tree, docs_grep, docs_read, team_bb_publish, etc.
        let reg = Arc::clone(&team_registry);
        let msg_smgglrs_addr = cfg.server.listen_addr();
        let msg_signer = Arc::clone(&root_signer);
        // Load ForgeService once and share across all teammate spawns
        let msg_forge: Option<Arc<smgglrs_cognitive::ForgeService>> = cfg.cognitive_core.as_ref().and_then(|p| {
        ...
        builder = builder.tool(team_tools::team_message_def(), move |args, _ctx| {
            let reg = Arc::clone(&reg);
            let smgglrs_addr = msg_smgglrs_addr.clone();
            ...
            Box::pin(async move {
                ...
                let bg_smgglrs_addr = smgglrs_addr.clone();
                ...
                let handle = tokio::spawn(async move {
                    ...
                    let mcp_url = format!("http://{bg_smgglrs_addr}/mcp");
                    ...
                    let teammate_cap = smgglrs_core::auth::capability::CapabilitySet {
                        paths: vec!["**".to_string()],
                    ...
                    let teammate_payload = smgglrs_core::auth::capability::build_payload(
                        bg_signer.did(),
                        &teammate_did,
                        teammate_cap,
                    ...
                    let teammate_token = match smgglrs_core::auth::capability::encode_token(
                        &teammate_payload,
                        bg_signer.as_ref(),
                    ) {
                        Ok(t) => t,
                    ...
                    macro_rules! run_teammate {
                        ($backend:expr) => {{
                            let r = async {
                                let mut agent = smgglrs_agent::Agent::builder()
                                    .endpoint(&mcp_url)
                                    .await?
                    ...
                    let agent_result = if is_claude {
                        ...
                        if use_vertex {
                            ...
                            run_teammate!(smgglrs_model::AnthropicBackend::new(
                                &url, &teammate_model, Some(token), smgglrs_model::Locality::Remote,
                            ))
                        } else {
        ...
        let fs_smgglrs_addr = cfg.server.listen_addr();
        ...
        let fs_forge: Option<Arc<smgglrs_cognitive::ForgeService>> = cfg.cognitive_core.as_ref().and_then(|p| {
            let expanded = expand_tilde(p);
        ...
        builder = builder.tool(
            ...
            move |args, ctx| {
                ...
                let smgglrs_addr = fs_smgglrs_addr.clone();
                    ...
                    tokio::spawn(async move {
                        ...
                        loop {
                            ...
                            for task in &ready {
                                ...
                                let spawn_addr = bg_smgglrs_addr.clone();
                                ...
                                let handle = tokio::spawn(async move {
                                    ...
                                    let result = tokio::time::timeout(deadline, async move {
                                        ...
                                        let token = match smgglrs_core::auth::capability::encode_token(&payload, spawn_signer.as_ref()) {
                                            Ok(t) => t,
                                            Err(e) => {
                                        ...
                                        let system_prompt = if let Some(ref pn) = tm_persona {
                                            spawn_forge.as_ref()
                                                .and_then(|f| smgglrs_cognitive::assemble(f, pn, "", None, None).ok())
                                        ...
                                        let r: Result<smgglrs_agent::ToolLoopResult, smgglrs_agent::AgentError> = async {
                            ...
                            for task in &ready {
                                ...
                                if new_tasks.is_empty() {
                                    ...
                                    let mcp_url = format!("http://{}/mcp", bg_smgglrs_addr);
        ...
        let fe_smgglrs_addr = cfg.server.listen_addr();
        let fe_signer = Arc::clone(&root_signer);
        let fe_budget_cfg = cfg.budget.clone();
        let fe_forge: Option<Arc<smgglrs_cognitive::ForgeService>> = cfg.cognitive_core.as_ref().and_then(|p| {
            let expanded = expand_tilde(p);
            smgglrs_cognitive::ForgeService::load(std::path::Path::new(&expanded)).ok().map(Arc::new)
        });
        builder = builder.tool(
            ...
            move |args, ctx| {
                ...
                let smgglrs_addr = fe_smgglrs_addr.clone();
                ...
                Box::pin(async move {
                    ...
                    loop {
                        ...
                        for task in &ready {
                            ...
                            let spawn_addr = smgglrs_addr.clone();
