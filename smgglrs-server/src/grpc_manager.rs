...
use smgglrs_core::grpc_module::{GrpcModule, GrpcModuleError};
...
mod tests {
    ...
    fn config_endpoint_from_socket() {
        let cfg = GrpcModuleConfig {
            name: "test".to_string(),
            command: vec![],
            socket: Some(PathBuf::from("/run/smgglrs/modules/test.sock")),
            address: None,
        ...
        assert_eq!(
            cfg.endpoint(),
            Some("unix:///run/smgglrs/modules/test.sock".to_string())
        );
    }
    ...
    fn config_deserialization() {
        let toml = r#"
name = "custom_tool"
command = ["/usr/libexec/smgglrs/modules/custom-tool"]
...
"#;
        ...
        assert_eq!(
            cfg.command,
            vec!["/usr/libexec/smgglrs/modules/custom-tool"]
        );
        assert_eq!(
            cfg.socket,
            Some(PathBuf::from("/run/smgglrs/modules/custom-tool.sock"))
        );
