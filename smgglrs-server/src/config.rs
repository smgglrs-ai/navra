...
"#;
    ...
    #[test]
    fn schema_is_valid_json_with_expected_fields() {
        ...
        // Root should have "properties" with top-level config fields
        let props = value["properties"].as_object().unwrap();
        assert!(props.contains_key("server"), "missing 'server' field");
        assert!(props.contains_key("modules"), "missing 'modules' field");
        assert!(props.contains_key("agents"), "missing 'agents' field");
        assert!(props.contains_key("permissions"), "missing 'permissions' field");
        assert!(props.contains_key("upstream"), "missing 'upstream' field");
        assert!(props.contains_key("models"), "missing 'models' field");
        assert!(props.contains_key("budget"), "missing 'budget' field");

        // Definitions should contain nested types
        let defs = value["definitions"].as_object().unwrap();
        assert!(defs.contains_key("ServerConfig"), "missing ServerConfig definition");
        assert!(defs.contains_key("ModulesConfig"), "missing ModulesConfig definition");
        assert!(defs.contains_key("BudgetConfig"), "missing BudgetConfig definition");
    }
