...
pub(crate) async fn run_demo(project: &str) -> anyhow::Result<()> {
    ...
    let identity_path = std::path::PathBuf::from("/tmp/smgglrs-demo/identity.key");
    std::fs::create_dir_all("/tmp/smgglrs-demo")?;
    let root_signer = smgglrs_core::identity::load_or_create_file_identity(&identity_path)?;
    ...
    let _forge = match smgglrs_cognitive::ForgeService::load(&project_path) {
        Ok(f) => {
    ...
    println!();
    println!("  Blackbox:    smgglrs audit");
    println!();
...
pub(crate) async fn run_demo_live(project: &str, model_name: &str, _max_rounds: u32, _files_per_round: usize, _min_delta: u32, custom_prompt: Option<&str>, writable: bool, allow_read: &[String], allow_write: &[String]) -> anyhow::Result<()> {
    ...
    // --- Step 2: Start smgglrs gateway ---
    println!("━━━ Act 1: Start smgglrs Gateway ━━━");
    ...
    // Write smgglrs config for the demo
    let demo_config_path = "/tmp/smgglrs-demo/agent-config.toml";
    std::fs::create_dir_all("/tmp/smgglrs-demo")?;
...
"#,
    ...
    let mut smgglrs_child = tokio::process::Command::new(&smgglrs_bin)
    ...
    // Wait for smgglrs to be ready
    let smgglrs_url = format!("http://127.0.0.1:{demo_port}");
    ...
    for i in 0..30 {
        ...
        if i == 29 {
            ...
            anyhow::bail!("smgglrs did not start within 15 seconds");
        }
    }
    println!("  ✓ smgglrs gateway running at {smgglrs_url}");
    ...
    match v1_check {
        Ok(resp) if resp.status().is_success() => {
            println!("  ✓ Model proxy at {smgglrs_url}/v1/chat/completions");
        }
        Ok(resp) => {
    ...
    println!();
    println!("━━━ Act 2: Cognitive Core ━━━");
    let forge = match smgglrs_cognitive::ForgeService::load(&project_path) {
        Ok(f) => {
            for name in f.persona_names() {
                if let Some(persona) = f.get_persona(name) {
                    println!(
    ...
    let base_prompt = if !persona_name.is_empty() {
        smgglrs_cognitive::assemble(&forge, persona_name, "", None, None)
    ...
    let mut agent = if is_claude {
        ...
        build_agent!(smgglrs_model::OpenAiBackend::new(
    ...
    println!("  ✓ Connected to smgglrs at {}", mcp_endpoint);
    ...
    println!("  the codebase through smgglrs. IFC, safety filters, and ACLs are");
    ...
    println!();
    smgglrs_child.kill().await?;
    println!("  ✓ smgglrs gateway stopped");
    println!();
