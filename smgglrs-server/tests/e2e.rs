...
//!
//! Each test spawns a real smgglrs server as a child process, connects
...
/// Spawn a smgglrs server with a given config, wait for it to be ready.
async fn spawn_smgglrs(config_toml: &str) -> (Child, u16, String) {
    // Pick a free port
    ...
    let config_path = format!("/tmp/smgglrs-e2e-{port}.toml");
    ...
    for i in 0..30 {
        ...
        if i == 29 {
            panic!("smgglrs did not start within 15 seconds");
        }
    }
...
async fn tool_call_recorded_in_blackbox() {
    let (mut child, _port, url) = spawn_smgglrs(DOCS_CONFIG).await;
    let client = reqwest::Client::new();
