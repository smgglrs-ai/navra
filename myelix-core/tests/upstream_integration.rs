...
use myelix_core::protocol::{CallToolParams, GetPromptParams};
use myelix_core::upstream::Upstream;
use myelix_core::UpstreamModule;
...
async fn upstream_spawn_and_discover_tools() {
    ...
    let tools: Vec<_> = myelix_core::Module::tools(&module);
...
async fn upstream_spawn_and_discover_prompts() {
    ...
    let prompts: Vec<_> = myelix_core::Module::prompts(&module);
...
async fn upstream_call_tool_through_module() {
    ...
    let tools = myelix_core::Module::tools(&module);
    let (_def, handler) = &tools[0];

    let ctx = myelix_core::auth::CallContext::new(
        myelix_core::auth::AgentIdentity::new("tester", "dev"),
        "test",
    );
    ...
    match &result.content[0] {
        myelix_core::protocol::Content::Text(t) => assert_eq!(t.text, "echo: hello"),
    }
}
...
async fn upstream_get_prompt_through_module() {
    ...
    let prompts = myelix_core::Module::prompts(&module);
...
async fn upstream_registers_in_server() {
    ...
    let server = myelix_core::McpServer::builder()
        .module(module)
        .build();

    // 1 upstream tool + 3 gateway tools (myelix_var_*)
