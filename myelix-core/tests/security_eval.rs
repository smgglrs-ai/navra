...
use myelix_core::auth::chain::{CapabilityAuthenticator, ChainAuthenticator};
use myelix_core::auth::{AgentIdentity, AuthError, Authenticator, TokenAuthenticator};
use myelix_core::identity::{CapSigner, Ed25519Signer};
use myelix_core::credentials::{CredentialMapping, CredentialStore, MappedCredentialStore};
...
mod escalation {
    ...
    fn delegation_cannot_escalate_ring() {
        ...
        let mut child = build_payload(
            "did:key:z6MkLeader", "did:key:z6MkEvil",
            parent.cap.clone(), 0, 3600,
        );
...
mod forgery {
    ...
    fn forged_token_with_wrong_key_rejected() {
        ...
        let payload = build_payload(
            ...
            CapabilitySet {
                paths: vec!["/**".to_string()],
                operations: vec!["read".to_string(), "write".to_string(), "shell.exec".to_string()],
                tools: vec!["*".to_string()],
    ...
    fn malformed_token_rejected() {
        ...
        assert!(decode_token("mcpd_cap_v1.bad-base64!.also-bad!", &signer).is_err());
        assert!(decode_token("mcpd_cap_v2.aaa.bbb", &signer).is_err()); // wrong version prefix
        assert!(decode_token("mcpd_cap_v1.onlytwoparts", &signer).is_err());
    }
    ...
    fn authenticator_rejects_forged_cap_token() {
        ...
        let payload = build_payload(
            root_signer.did(), "did:key:z6MkEvil",
            CapabilitySet {
                paths: vec!["/**".to_string()],
...
mod credential_isolation {
    ...
    fn only_configured_labels_accessible() {
        ...
        mappings.insert("allowed".to_string(), CredentialMapping {
    ...
    fn capability_token_credential_subset() {
        ...
        let mut child = build_payload(
            "did:key:z6MkLeader", "did:key:z6MkSpecialist",
            CapabilitySet {
                paths: vec![],
                operations: vec![],
                tools: vec![],
    ...
    fn credential_labels_not_values_in_token() {
        ...
        assert!(token.contains("github") == false || {
    ...
    fn cannot_store_to_env_credential() {
        let mut mappings = HashMap::new();
        mappings.insert("env_cred".to_string(), CredentialMapping {
            source: "env".to_string(),
            path: None,
...
mod attenuation {
    ...
    fn valid_attenuation_chain() {
        ...
        let root_to_leader = build_payload(
            ...
            CapabilitySet {
                paths: vec!["/home/**".to_string()],
                operations: vec!["read".to_string(), "write".to_string(), "git.commit".to_string()],
                tools: vec!["*".to_string()],
        ...
        let mut leader_to_spec = build_payload(
            "did:key:z6MkLeader", "did:key:z6MkSpec",
            CapabilitySet {
    ...
    fn empty_capabilities_always_valid_delegation() {
        ...
        let mut child = build_payload(
            "did:key:z6MkParent", "did:key:z6MkChild",
            CapabilitySet {
                paths: vec![],
                operations: vec![],
                tools: vec![],
...
mod audit {
    ...
    fn token_contains_issuer_and_subject_did() {
        let signer = Ed25519Signer::generate();
        let payload = build_payload(
            signer.did(), "did:key:z6MkSubject",
            CapabilitySet {
                paths: vec![], operations: vec![], tools: vec![], credentials: vec![],
            },
            1, 3600,
...
mod auth_chain {
    ...
    fn chain_falls_through_correctly() {
        ...
        let cap_payload = build_payload(
            root.did(), "did:key:z6MkCap",
            CapabilitySet {
                paths: vec![], operations: vec!["read".to_string()],
                tools: vec!["*".to_string()], credentials: vec![],
            },
            1, 3600,
        );
        ...
        h1.insert("authorization", format!("Bearer {cap_token}").parse().unwrap());
...
mod quota {
    use myelix_core::quota::{QuotaEngine, RateLimit};

    #[test]
    fn rate_limit_enforced() {
        let mut engine = QuotaEngine::new();
        engine.add_limit("restricted".to_string(), RateLimit {
            max_calls: 5,
            window_secs: 60,
        });
    ...
    fn rate_limit_per_agent_isolation() {
        let mut engine = QuotaEngine::new();
        engine.add_limit("dev".to_string(), RateLimit {
            max_calls: 2,
            window_secs: 60,
        });
