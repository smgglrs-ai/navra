...
use myelix_core::auth::{AgentIdentity, CallContext};
use myelix_core::ifc::{DataLabel, TaintedWritePolicy};
use myelix_core::protocol::{
    CallToolParams, CallToolResult, ClientInfo, Content, InitializeParams,
    ToolDefinition, ToolInputSchema,
};
use myelix_core::McpServer;
...
async fn clean_write_allowed_after_tainted_read() {
    ...
    // Use myelix_var_list (gateway tool, excluded from is_external_read_tool
...
/// myelix_var_list returns stored variables with labels.
#[tokio::test]
...
async fn context_label_persists_across_calls() {
    ...
    assert_eq!(
        ...
        myelix_core::ifc::Integrity::Untrusted,
...
impl OllamaContainer {
    ...
    fn start() -> Option<Self> {
        ...
        let name = format!("myelix-test-ollama-{}", std::process::id());
...
///
/// Run with: cargo test -p myelix-core --test ifc_integration llm -- --ignored
#[tokio::test]
#[ignore]
