...
use mcpd_core::{Module, ToolHandler};
...
async fn handle_listen(
    ...
    // Transcribe
    let request = mcpd_core::models::TranscribeRequest {
        audio,
        language,
    };
...
async fn handle_speak(
    ...
    // Synthesize
    let request = mcpd_core::models::SynthesizeRequest {
        text: text.to_string(),
        voice,
    };
...
async fn handle_transcribe(
    ...
    let request = mcpd_core::models::TranscribeRequest { audio, language };
    match state.asr_model.transcribe(&request).await {
        Ok(response) => {
            let mut output = response.text.clone();
            if let Some(lang) = &response.language {
