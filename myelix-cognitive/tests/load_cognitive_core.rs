...
use myelix_cognitive::{assemble, ForgeService};
use std::path::Path;
...
fn assemble_software_developer() {
    let forge = ForgeService::load(&cognitive_core_path()).unwrap();
    let output = assemble(&forge, "software_developer", "Fix the login bug", None, None).unwrap();
    let prompt = output.system_prompt();
    assert!(prompt.contains("Software Developer"), "missing display name");
    assert!(prompt.len() > 500, "prompt too short: {} chars", prompt.len());
...
fn assemble_guardian_loads_directives() {
    let forge = ForgeService::load(&cognitive_core_path()).unwrap();
    let output = assemble(&forge, "myelix_guardian", "Check system", None, None).unwrap();
    let prompt = output.system_prompt();
    assert!(prompt.contains("Core Directives"), "guardian should load directives");
}
...
fn all_personas_assemble_without_error() {
    ...
    for name in forge.persona_names() {
        ...
        assert!(result.is_ok(), "persona '{}' failed to assemble: {:?}", name, result.err());
    }
}
