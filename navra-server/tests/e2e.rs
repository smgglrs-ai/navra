...
//!
//! Each test spawns a real navra server as a child process, connects
...
async fn spawn_navra(config_toml: &str) -> (Child, u16, String) {
    ...
    let config_path = format!("/tmp/navra-e2e-{port}.toml");
    let config = config_toml.replace("{port}", &port.to_string());
    std::fs::write(&config_path, &config).unwrap();

    // Find the navra binary
...
async fn tool_call_recorded_in_blackbox() {
    ...
    // The blackbox is at ~/.local/share/navra/blackbox.db
