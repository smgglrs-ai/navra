...
//!
//! These tests verify the combined OpenShell + navra security model
...
//!     cargo test -p navra-server --test openshell_integration -- --ignored
...
/// navra's ACLs (Layer 2 / DAC) also prevent unauthorized tool
...
async fn agent_cannot_reach_unauthorized_endpoint() {
    ...
    // 1. Start navra with restrictive ACLs (no network tools)
    // 2. Attempt to call a tool that reaches the unauthorized endpoint
    // 3. Verify navra blocks it at the permission layer
    //
    // The key insight: even if the network namespace were bypassed,
    // navra's ACLs prevent the agent from using tools that could
...
///
/// This tests the DAC (navra) layer of defense in depth:
...
async fn authorized_tool_calls_respect_acls() {
    // In a real test:
    // 1. Start navra with a config that allows "read" on /tmp/test-workspace/**
    //    and denies "write"
...
/// navra should reject the write-down.
///
...
async fn ifc_taint_propagation_across_a2a_boundary() {
    use navra_core::ifc::TaintTracker;
    use navra_core::protocol::label::{Confidentiality, DataLabel};
...
async fn openshell_identity_token_accepted() {
    ...
    let _config = navra_core::auth::openshell::OpenShellAuthConfig {
        mode: navra_core::auth::openshell::OpenShellAuthMode::Local,
...
async fn scoped_capability_token_limits_operations() {
    ...
    let signer = navra_core::identity::Ed25519Signer::generate();
    assert!(
...
/// agent somehow obtains PII from a tool, navra's safety filters
/// prevent it from persisting or exfiltrating the raw data.
#[tokio::test]
async fn pii_filter_applied_to_cross_sandbox_data() {
    use navra_core::safety::{ContentFilter, FilterContext, PiiFilter};
