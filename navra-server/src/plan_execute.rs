...
/// Provides `call_tool()` for sandboxed Python scripts to invoke navra tools.
const BRIDGE_PY: &str = r#""""navra tool bridge for sandboxed Python execution."""
import json, os, sys, urllib.request

GATEWAY_URL = os.environ.get("NAVRA_GATEWAY", "http://host.containers.internal:9400")
SESSION_ID = os.environ.get("NAVRA_SESSION", "")
AUTH_TOKEN = os.environ.get("NAVRA_TOKEN", "")

_call_id = 0

def call_tool(name, arguments=None):
    """Call an navra tool by name. Returns the text content of the result."""
    global _call_id
    _call_id += 1
    payload = json.dumps({
        "jsonrpc": "2.0",
...
/// `NAVRA_ALLOW_DIRECT_EXECUTION` environment variable is set to `true`.
fn is_direct_execution_allowed(allow_direct: bool) -> bool {
    if allow_direct {
        return true;
    }
    std::env::var("NAVRA_ALLOW_DIRECT_EXECUTION")
    ...
    if is_python3_available() {
        ...
        tracing::info!(
            "Python 3 is available but direct execution is disabled. \
             ...
             NAVRA_ALLOW_DIRECT_EXECUTION=true to enable."
        );
    }
...
/// Execute a Python script in a sandbox with access to navra tools.
///
...
pub async fn execute_python(
        ...
        None => {
            return CallToolResult::error(
                "Python mode requires a sandbox (OpenShell or Podman) but none is \
                 ...
                 NAVRA_ALLOW_DIRECT_EXECUTION=true) to allow unsandboxed execution.",
            );
        }
    };
    ...
    let full_script = format!(
        "import sys\nsys.path.insert(0, '{work_dir}')\nfrom navra_bridge import call_tool\n\n{code}",
    ...
    let host_addr =
        std::env::var("NAVRA_LISTEN_ADDR").unwrap_or_else(|_| "127.0.0.1:9315".to_string());
    ...
    let auth_token = std::env::var("NAVRA_SANDBOX_TOKEN").unwrap_or_default();
...
async fn execute_in_openshell(
    ...
    cmd.arg("run")
        ...
        .arg("-e")
        .arg(format!("NAVRA_GATEWAY={gateway_url}"))
        .arg("-e")
        .arg(format!("NAVRA_SESSION={session_id}"))
        .arg("-e")
        .arg(format!("NAVRA_TOKEN={auth_token}"))
        .arg("-w")
        .arg("/work")
        // OpenShell label annotations (passed through to the container)
        .arg("--label")
        .arg("navra.runtime=python3")
        .arg("--label")
        .arg("navra.purpose=plan_execute")
        .arg("python:3-slim")
        .arg("python")
        .arg("/work/script.py");
...
async fn execute_direct(
    ...
    cmd.arg(&script_path)
        .env("NAVRA_GATEWAY", gateway_url)
        .env("NAVRA_SESSION", session_id)
        .env("NAVRA_TOKEN", auth_token)
...
"#;
    ...
    fn test_python_env_setup() {
        ...
        assert!(BRIDGE_PY.contains("NAVRA_GATEWAY"));
        assert!(BRIDGE_PY.contains("NAVRA_SESSION"));
        assert!(BRIDGE_PY.contains("NAVRA_TOKEN"));
    }
...
"#;
    ...
    async fn test_direct_mode_blocked_by_default() {
        ...
        assert!(
            text.contains("allow_direct_execution")
                || text.contains("NAVRA_ALLOW_DIRECT_EXECUTION"),
    ...
    fn test_ctx() -> CallContext {
        CallContext {
            agent: navra_core::auth::AgentIdentity::new("test-agent", "readonly"),
            session_id: "test-session".to_string(),
            taint: navra_core::ifc::TaintTracker::new(),
