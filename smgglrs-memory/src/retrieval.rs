...
//! - Vector: semantic similarity via `smgglrs-rag` ChunkStore (requires `rag` feature)
...
pub struct MemoryRetriever<'a> {
    ...
    #[cfg(feature = "rag")]
    chunk_store: Option<Arc<smgglrs_rag::ChunkStore>>,
}
...
impl<'a> MemoryRetriever<'a> {
    ...
    #[cfg(feature = "rag")]
    pub fn with_chunk_store(mut self, chunk_store: Arc<smgglrs_rag::ChunkStore>) -> Self {
    ...
    /// text using whatever model is appropriate (e.g. via `smgglrs-model`).
