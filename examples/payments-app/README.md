...
End-to-end demonstration of the myelix-* framework performing a
...
| 10 | **Human-in-the-Loop** | `git_commit` requires approval. D-Bus notification sent to desktop. User approves via system tray or `mcpd approve <id>`. |
...
└── config/                       # mcpd + flow configuration
    ├── demo-config.toml           # mcpd gateway config
...
# Run the demo (future: mcpd demo --project examples/payments-app)
mcpd serve --config examples/payments-app/config/demo-config.toml
```
